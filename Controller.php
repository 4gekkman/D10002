 ...
 *
 *                  POST-API1   D10002:1              (v)      Принять данные FormData, передать команде C1_saveimage, вернуть ответ
 *                  POST-API2   D10002:2              (v)      Описание
 *
 *
 *
 */
...
class Controller extends BaseController {
  ...
  public function postIndex() {
    ...
    //------------------------//
      ...
      //------------------------------------------//
      // - $key       - ключ операции (напр.: D10002:1)
      ...
      if(!empty($key) && empty($command)) {
        ...
        //-----------------------------//
        // Нестандартная операция D10002:1 //
        //-----------------------------//
        if($key == 'D10002:1') {

          // 1. Подготовить полный путь к команде
          $command = "\\M7\\Commands\\C1_saveimage";

          // 2. Подготовить данные для команды
          $data = [
          ...
          // 3. Выполнить команду и получить результаты
          $response = runcommand(
          ...
          // 4. Добавить к $results значение timestamp поступления запроса
          $response['timestamp'] = $data['timestamp'];
